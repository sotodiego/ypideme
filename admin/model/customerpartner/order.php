...
class ModelCustomerpartnerOrder extends Model {
	public function getOrderCopy($order_id) {
...
/**
  ...
  /**
	...
	public function getOrder($order_id) {
		...
		if ($order_query->num_rows) {
			...
			return array(
				...
				'shipping_applied'             => isset($order_query->row['shipping_applied']) ? $order_query->row['shipping_applied'] : 0 ,
	...
	public function getSellerLogo($product_id) {
		$seller_id = 0;
		if($product_id)
		  $seller_id = $this->getSellerbasedonProduct($product_id);
		return $this->db->query("SELECT companylogo FROM " . DB_PREFIX . "customerpartner_to_customer WHERE customer_id = '" . (int)$seller_id . "'")->row['companylogo'];
	}
