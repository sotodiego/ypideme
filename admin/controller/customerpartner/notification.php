...
class ControllerCustomerpartnerNotification extends Controller {
	...
	public function index() {
		...
		$data = array_merge($data, $this->load->language('customerpartner/notification'));
  	...
  	$data['breadcrumbs'][] = array(
		...
		if ($seller_notifications) {
			foreach ($seller_notifications as $key => $seller_notification) {
				...
				if ($seller_notification['key'] == 'order_account') {
					...
					$data['seller_notifications'][] = sprintf($this->language->get('text_order_add'),$this->session->data['user_token'],$seller_notification['data']['order_id'],$seller_notification['data']['order_id'],$seller_notification['data']['name'],$seller_notification['date_added']);
					...
					if ($order_id) {
						$data['seller_notifications'][] = sprintf($this->language->get('text_order_return'),$seller_notification['data']['name'],$this->session->data['user_token'],$order_id['order_id'],$seller_notification['data']['return_id'],$order_id['product'],$seller_notification['date_added']);
					}
					...
					if ($status) {
						if (empty($data['selected']) || in_array('all',$data['selected'])) {
						  $data['seller_notifications'][] = sprintf($this->language->get('text_order_status'),$this->session->data['user_token'],$seller_notification['data']['order_id'],$seller_notification['data']['order_id'],$status['name'],$seller_notification['date_added']);
						} else {
		...
		if ($seller_product_reviews) {
			foreach ($seller_product_reviews as $key => $seller_product_review) {
				...
				if ($seller_product_review['key'] == 'product_review') {
					$data['seller_product_reviews'][] = sprintf($this->language->get('text_product_review'),$seller_product_review['id'],$seller_product_review['data']['author'],$this->session->data['user_token'],$seller_product_review['data']['product_name'],$seller_product_review['data']['product_name'],$seller_product_review['date_added']);
				} elseif($seller_product_review['key'] == 'product_stock') {
					$data['seller_product_reviews'][] = sprintf($this->language->get('text_product_stock'),$this->session->data['user_token'],$seller_product_review['data']['product_name'],$seller_product_review['data']['product_name'],$seller_product_review['date_added']);
				} elseif ($seller_product_review['key'] == 'product_approve') {
					$data['seller_product_reviews'][] = sprintf($this->language->get('text_product_approve'),$this->session->data['user_token'],$seller_product_review['data']['product_name'],$seller_product_review['data']['product_name'],$seller_product_review['date_added']);
				}
			}
		}
		...
		if ($seller_reviews) {
			foreach ($seller_reviews as $key => $seller_review) {
				...
				if (isset($date_diff['y']) && $date_diff['y']) {
					...
					$data['seller_reviews'][] = sprintf($this->language->get('text_seller_review'),$seller_review['id'],$this->session->data['user_token'],$seller_review['name'],$seller_review['name'],$seller_review['createdate']);
			}
		}
		...
		if ($categories) {
		  foreach ($categories as $key => $category) {
		    ...
		    if (isset($category['data']['category_name']) && $category['data']['category_name']) {
		      $data['seller_reviews'][] = sprintf($this->language->get('text_category_approve'),$this->session->data['user_token'],$category['data']['category_name'],$category['date_added']);
		    }
		  }
		}
		...
		$pagination->limit = 10;
		$pagination->url = $this->url->link('customerpartner/notification','user_token=' . $this->session->data['user_token'].$url.'&page={page}&page_product='.$page_product.'&page_seller='.$page_seller, true);
		...
		$pagination_product->url = $this->url->link('customerpartner/notification','user_token=' . $this->session->data['user_token'].$url.'&page_product={page}&page='.$page.'&page_seller='.$page_seller . '&tab=product', true);
		...
		$pagination_seller->url = $this->url->link('customerpartner/notification','user_token=' . $this->session->data['user_token'].$url.'&page_seller={page}&page='.$page.'&page_product='.$page_product . '&tab=seller', true);
		...
		$this->response->setOutput($this->load->view('customerpartner/notification', $data));
	}
	...
	public function notifications()	{
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
			if (isset($this->session->data['user_token']) && $this->config->get('module_marketplace_status') && $this->db->query("SELECT * FROM information_schema.TABLES WHERE TABLE_SCHEMA = '".DB_DATABASE."' AND (TABLE_NAME = '".DB_PREFIX."seller_activity' || TABLE_NAME = '".DB_PREFIX."mp_customer_activity')")->row) {
				...
				$data['view_all'] = $this->url->link('customerpartner/notification','user_token='.$this->session->data['user_token'],true);

				$data['seller_notifications'] = array();
				if(isset($this->request->server['HTTP_REFERER']) && $this->request->server['HTTP_REFERER'] && strpos($this->request->server['HTTP_REFERER'],'module/journal2') !== false) {
					$data['journal_page'] = true;
				}
				...
				if ($seller_notifications) {
					foreach ($seller_notifications as $key => $seller_notification) {
						...
						if ($seller_notification['key'] == 'order_account') {
							$seller_notification['data'] = json_decode($seller_notification['data'],1);
							$data['seller_notifications'][] = sprintf($this->language->get('text_order_add_mp'),$this->session->data['user_token'],$seller_notification['data']['order_id'],$seller_notification['data']['order_id'],$seller_notification['data']['name'],$seller_notification['date_added']);
						} elseif ($seller_notification['key'] == 'return_account') {
							$seller_notification['data'] = json_decode($seller_notification['data'],1);
							$order_id = $this->model_customerpartner_notification->getReturnOrderId($seller_notification['data']['return_id']);
							if ($order_id) {
								$data['seller_notifications'][] = sprintf($this->language->get('text_order_return_mp'),$seller_notification['data']['name'],$this->session->data['user_token'],$order_id['order_id'],$seller_notification['data']['return_id'],$order_id['product'],$seller_notification['date_added']);
							}
						} elseif ($seller_notification['key'] == 'order_status') {
							$seller_notification['data'] = json_decode($seller_notification['data'],1);
							$status = $this->model_localisation_order_status->getOrderStatus($seller_notification['data']['status']);
							$data['seller_notifications'][] = sprintf($this->language->get('text_order_status_mp'),$this->session->data['user_token'],$seller_notification['data']['order_id'],$seller_notification['data']['order_id'],$status['name'],$seller_notification['date_added']);
						}
					}
				}
				...
				if ($seller_product_reviews) {
					foreach ($seller_product_reviews as $key => $seller_product_review) {
						...
						$seller_product_review['data'] = json_decode($seller_product_review['data'],1);
						if ($seller_product_review['key'] == 'product_review') {
							$data['seller_product_reviews'][] = sprintf($this->language->get('text_product_review_mp'),$seller_product_review['id'],$seller_product_review['data']['author'],$this->session->data['user_token'],$seller_product_review['data']['product_name'],$seller_product_review['data']['product_name'],$seller_product_review['date_added']);
						} elseif($seller_product_review['key'] == 'product_stock') {
							$data['seller_product_reviews'][] = sprintf($this->language->get('text_product_stock'),$this->session->data['user_token'],$seller_product_review['data']['product_name'],$seller_product_review['data']['product_name'],$seller_product_review['date_added']);
						} elseif ($seller_product_review['key'] == 'product_approve') {
							$data['seller_product_reviews'][] = sprintf($this->language->get('text_product_approve'),$this->session->data['user_token'],$seller_product_review['data']['product_name'],$seller_product_review['data']['product_name'],$seller_product_review['date_added']);
				...
				if ($seller_reviews) {
					foreach ($seller_reviews as $key => $seller_review) {
						...
						$data['seller_reviews'][] =
							sprintf($this->language->get('text_seller_review_mp'),$seller_review['id'],$this->session->data['user_token'],$seller_review['name'],$seller_review['name'],$seller_review['createdate']);
					}
				}
				...
				$this->response->setOutput(json_encode($this->load->view('customerpartner/notification_header', $data)));
			}
		} else {
