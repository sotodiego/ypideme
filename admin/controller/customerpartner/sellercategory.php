...
class ControllerCustomerpartnerSellercategory extends Controller {
	...
	public function index() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('customerpartner/sellercategory', 'user_token=' . $this->session->data['user_token'] . $url, true)
		);
		...
		foreach ($results as $result) {
			$data['categories'][] = array(
				...
				'edit'        => $this->url->link('catalog/category/edit', 'user_token=' . $this->session->data['user_token'] . '&category_id=' . $result['category_id'] . '&mpcheck=1' . $url, true),
				'delete'      => $this->url->link('customerpartner/sellercategory/delete', 'user_token=' . $this->session->data['user_token'] . '&category_id=' . $result['category_id'] . $url, true)
			);
		}
		...
		$data['sort_name'] = $this->url->link('customerpartner/sellercategory', 'user_token=' . $this->session->data['user_token'] . '&sort=name' . $url, true);
		$data['sort_sort_order'] = $this->url->link('customerpartner/sellercategory', 'user_token=' . $this->session->data['user_token'] . '&sort=sort_order' . $url, true);
		...
		$pagination->url = $this->url->link('customerpartner/sellercategory', 'user_token=' . $this->session->data['user_token'] . $url . '&page={page}', true);
		...
		$this->response->setOutput($this->load->view('customerpartner/sellercategory_list', $data));
	}
