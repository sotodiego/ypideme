...
class ControllerCustomerpartnerReviewfield extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('customerpartner/reviewfield', 'user_token=' . $this->session->data['user_token'] . $url, true));
		}
	...
	protected function getList($data) {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('customerpartner/reviewfield', 'user_token=' . $this->session->data['user_token'] . $url, true)
		);

		$data['add'] = $this->url->link('customerpartner/reviewfield/add', 'user_token=' . $this->session->data['user_token'] . $url, true);
		$data['delete'] = $this->url->link('customerpartner/reviewfield/delete', 'user_token=' . $this->session->data['user_token'] . $url, true);
		...
		foreach ($results as $result) {
			$data['reviewfields'][] = array(
				...
				'edit'             => $this->url->link('customerpartner/reviewfield/edit', 'user_token=' . $this->session->data['user_token'] . '&reviewfield_id=' . $result['field_id'] . $url, true)
			);
		}
		...
		$pagination->url = $this->url->link('customerpartner/reviewfield', 'user_token=' . $this->session->data['user_token'] . $url . '&page={page}', true);
		...
		$this->response->setOutput($this->load->view('customerpartner/reviewfield_list', $data));
	}
	...
	protected function getForm($data) {
		...
		if (!isset($this->request->get['reviewfield_id'])) {
			$data['action'] = $this->url->link('customerpartner/reviewfield/add', 'user_token=' . $this->session->data['user_token'] . $url, true);
		} else {
			$data['action'] = $this->url->link('customerpartner/reviewfield/edit', 'user_token=' . $this->session->data['user_token'] . '&reviewfield_id=' . $this->request->get['reviewfield_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('customerpartner/reviewfield', 'user_token=' . $this->session->data['user_token'] . $url, true);
		...
		$this->response->setOutput($this->load->view('customerpartner/reviewfield_form', $data));
	}
