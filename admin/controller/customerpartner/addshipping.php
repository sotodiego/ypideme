		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			if(isset($files['up_file']['tmp_name']) AND $files['up_file']['tmp_name']){
				...
				if(isset($csv_extention[1]) AND $csv_extention[1] == 'csv'){
					...
					$this->response->redirect($this->url->link('customerpartner/addshipping/matchdata', 'user_token=' . $this->session->data['user_token'], true));
				}else{
 		...
 		if (isset($this->error['warning'])) {
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('customerpartner/addshipping', 'user_token=' . $this->session->data['user_token'], true),
      		'separator' => ' :: '
   		);

		$this->data['back'] = $this->url->link('customerpartner/shipping', 'user_token=' . $this->session->data['user_token'], true);
		$this->data['action'] = $this->url->link('customerpartner/addshipping', 'user_token=' . $this->session->data['user_token'], true);
		$this->data['header'] = $this->load->controller('common/header');
		$this->data['footer'] = $this->load->controller('common/footer');
		$this->data['column_left'] = $this->load->controller('common/column_left');
		$this->response->setOutput($this->load->view('customerpartner/addshipping',$this->data));
	...
	public function stepTwo($fields = array()) {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $fields == array()) {
			...
			if(isset($this->error['warning']) AND $this->error['warning']){
				...
				$this->response->redirect($this->url->link('customerpartner/addshipping', 'user_token=' . $this->session->data['user_token'], true));
		...
		$this->data['action'] = $this->url->link('customerpartner/addshipping/stepTwo', 'user_token=' . $this->session->data['user_token'], true);

		$this->data['cancel'] = $this->url->link('customerpartner/addshipping', 'user_token=' . $this->session->data['user_token'], true);

		$this->data['header'] = $this->load->controller('common/header');
		$this->data['footer'] = $this->load->controller('common/footer');
		$this->data['column_left'] = $this->load->controller('common/column_left');
		$this->response->setOutput($this->load->view('customerpartner/addshipping_next',$this->data));
