...
class ModelCustomerpartnerReason extends Model {
  ...
  public function addSellerReasonEntry($reason_id)   {
    $this->db->query("INSERT INTO `" . DB_PREFIX . "customerpartner_to_reason` SET `reason_id` = " . (int)$reason_id . ",seller_id = '".(int)$this->customer->getId()."'");
  }
  ...
  public function addReasonDescription($reason_id ,$data) {
	...
	public function getreason($reason_id) {
		$query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "mp_reason mr  LEFT JOIN " . DB_PREFIX . "customerpartner_to_reason c2r ON (mr.reason_id = c2r.reason_id) WHERE mr.reason_id = '" . (int)$reason_id . "' AND c2r.seller_id = '".(int)$this->customer->getId()."'");
		return $query->row;
	}
