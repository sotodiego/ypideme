...
class ControllerCustomerpartnerAutocomplete extends Controller {
	...
	public function categoryOptions() {
		...
		if (isset($this->request->get['category_id']) && $this->request->get['category_id']) {
			$this->load->model('account/categorymapping');

			$results = $this->model_account_categorymapping->getCategoryOptions($this->request->get['category_id']);

			if ($results) {
				foreach ($results as $vlk =>$result) {
					if (isset($result['option_id']) && $result['option_id']) {
						$this->language->load('account/customerpartner/addproduct');
						$this->load->model('account/customerpartner');
						$this->load->model('tool/image');

						$option = $this->model_account_categorymapping->getOption($result['option_id']);
						 ...
						 $option_value_data = array();
							...
							$json[$vlk] = array(
								...
								'label'        => strip_tags(html_entity_decode($option['name'], ENT_QUOTES, 'UTF-8')),
								'category'     => $type,
								'type'         => $option['type'],
								'option_value' => $option_value_data
							);
