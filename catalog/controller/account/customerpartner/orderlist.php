...
class ControllerAccountCustomerpartnerOrderlist extends Controller {
	...
	public function index() {
			...
			foreach($orders as $key => $order_list){
				...
				if ($totals && isset($totals[0]['total'])) {
					...
					if (isset($totals[0]['total'])) {
						$orders[$key]['total'] = $this->currency->format($totals[0]['total'] ,$orders[$key]['currency_code'],$orders[$key]['currency_value']);
					}
				}
		...
		$this->data['marketplace_seller_create_order'] = $this->config->get('marketplace_seller_create_order');
		$this->data['add_order'] = $this->url->link('account/customerpartner/add_order','',true);
